...
class SpreadOutwardPattern(PatternBase):
    ...
    def pattern(self):
        hue = random()
        hue_offset = .08
        gr = Gradient([(0.0, self.colors.hue_to_color(hue)),
                       (.3333, self.colors.hue_to_color(hue + hue_offset)),
                       (.6666, self.colors.hue_to_color(hue + hue_offset * 2)),
                       (1.0, self.colors.hue_to_color(hue + hue_offset * 3))])
        stop = False
        while not stop:
            ...
            for radius100 in range(-100, 100 * (NUM_RINGS + 1), 10):
                ...
                for ring in range(-1, NUM_RINGS + 1):
                    ...
                    self.trap.set_color(group(ring), color,
                                        6)  #(int(color[0] * value), int(color[1] * value), int(color[2] * value)))
