...
class HippieTrap(object):
    ...
    def __enter__(self):
        ...
        try:
            check_call(["raspi-gpio", "set", "14", "a5"])
        except CalledProcessError as err:
            log("cannot call raspi-gpio:", str(err))
            sys.exit(-1)
        ...
        try:
            ...
            log("Cant open serial port: %s" % self.device)
            sys.exit(-1)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            check_call(["raspi-gpio", "set", "14", "op"])
        except CalledProcessError as err:
            log("cannot call raspi-gpio:", str(err))
            sys.exit(-1)
    ...
    def power_on(self):
        ...
        log("wait 2 seconds for full startup")
        sleep(2)
